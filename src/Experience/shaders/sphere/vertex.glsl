...
vec3 fad3(vec3 t) {return t*t*t*(t*(t*6.0-15.0)+10.0);}
...
uniform vec3 uLightAColour;
uniform vec3 uLightAPosition;
uniform float uLightAIntensity;

uniform vec3 uLightBColour;
...
varying float mathPI;

float mathPi = 3.1415926535897932384626433832795;
...
void main() {
  ...
  // Bi tangents
  float distanceA = (mathPi * 2.0) / uSubdivision;
  float distanceB = mathPi / uSubdivision;
  ...
  colour = mix(colour, uLightAColour, lightAIntensity * fresnel);
  colour = mix(colour, uLightBColour, lightBIntensity * fresnel);
  colour = mix(colour, vec3(1.0), clamp(pow(max(0.0, fresnel - 0.8), 3.0), 0.0, 1.0));
